...
class bkgTask(QRunnable):
    ...
    def run(self):
        ...
        while(runthread):
            ...
            mutex.lock()
            detection_output = YOLO_model.predict(source=img, conf=0.3, save=False,device='cpu')
            detection_output_Person = Yolo_model_person_detector.predict(source=img, conf=0.3, save=False,device='cpu')
            mutex.unlock()
            ...
            if self.skipthisiteration==False:
                ...
                current_output=detection_output[0]
                ...
                for old_coordinate in old_image_output.boxes:
                    ...
                    if Matchfound==False:   
                        ...
                        checkpoint = torch.load('Models\\Resnet34Model.pth', map_location=device)
                        ...
                        #Crop image
                        img_cropped = self.OLD_IMAGE[int(y1_old+50):int(y2_old-50), int(x1_old+50):int(x2_old-50)]
            ...
            self.OLD_IMAGE_COORDINATES=detection_output
            self.OLD_IMAGE=img
                            ...
                            if self.mainwindow.ui.ModelVision.isChecked()==True:
                                                                ...
                                                                while True:
                                                                    try:
                                                                        ...
                                                                        #run model on image
                                                                        detection_output_Objects = YOLO_model.predict(source=img, conf=0.3, save=False,device='cpu')
                                                                        detection_output_Person = Yolo_model_person_detector.predict(source=img, conf=0.3, save=False,device='cpu')
                                                                        
                                                                        result=detection_output_Objects[0]
                                                                        ...
                                                                        for box in person_result.boxes:
                                                                            ...
                                                                            if class_id == 0.0:
                                                                                ...
                                                                                detections.xyxy=np.append(detections.xyxy,values=[(np.array(person_result.boxes[counter].xyxy.tolist(),dtype="float32")).reshape(4,)],axis=0)
                                                                                detections.confidence=np.append(detections.confidence,values=[(np.array(person_result.boxes[counter].conf.tolist(),dtype="float32"))])

                                                                                
                                                                                new_list = [x+1 for x in person_result.boxes[counter].cls.tolist()]
                                                                                detections.class_id=np.append(detections.class_id,values=[(np.array(new_list,dtype='int32'))])
                                                                            counter+=1
                            ...
                            if self.mainwindow.ui.ModelVision.isChecked()==True:
                                                                ...
                                                                while True:
                                                                    try:
                                                                        ...
                                                                        #run model on image
                                                                        detection_output_Objects = YOLO_model.predict(source=img, conf=0.6, save=False,device='cpu')
                                                                        detection_output_Person = Yolo_model_person_detector.predict(source=img, conf=0.3, save=False,device='cpu')
